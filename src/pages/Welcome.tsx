...
const sectionCards = [
  { title: 'BATCH\nMANAGEMENT', bg: 'bg-[#22C55E]' },
  { title: 'FEED\nCALCULATOR', bg: 'bg-[#06B6D4]' },
  { title: 'HEALTH &\nVACCINATION', bg: 'bg-[#4A90D9]' },
  { title: 'EGG PRODUCTION\n& TRACKING', bg: 'bg-[#F59E0B]' },
  { title: 'FINANCE\n& STOCK', bg: 'bg-[#F97316]' },
  { title: 'OFFLINE-FIRST\nSYNC', bg: 'bg-[#22C55E]' },
];
...
export default function Welcome() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0EB] text-foreground font-sans">
        ...
        <div className="mx-auto flex h-12 max-w-[1400px] items-center justify-between px-5 lg:px-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.15em]">
              {menuOpen ? <><X className="h-3 w-3" /> Close</> : <>Menu <span className="text-primary font-black">+</span></>}
            </button>
          </div>
        </div>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            className="border-t border-black/5 px-5 py-4 flex flex-col gap-3 bg-[#F5F0EB]">
      ...
      <section className="relative min-h-[calc(100vh-3rem)] overflow-hidden">
        <div className="absolute inset-0 grid grid-cols-2 lg:grid-cols-[1.2fr_0.8fr_1fr]">
          ...
          <div className="hidden lg:block relative overflow-hidden">
            <img src={IMG.landscape} alt="West African farming landscape" className="absolute inset-0 w-full h-full object-cover" />
      ...
      {/* ═══ BREADCRUMB ═══ */}
      <div className="bg-white px-5 lg:px-10 py-3 border-y border-black/5">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">
        ...
        <div className="max-w-[1400px] mx-auto px-5 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-10 lg:gap-16 items-start">
            <motion.div {...fadeUp} className="relative">
              <img src={IMG.portrait1} alt="Farm technology expert" className="w-full aspect-square object-cover rounded-sm" loading="lazy" />
              <div className="absolute -bottom-2 left-0 w-full h-3 bg-[#F59E0B]/80" />
            </motion.div>
            <motion.div {...fadeUp}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-10 lg:gap-16 items-start">
            <motion.div {...fadeUp}>
              ...
              <div className="mt-6 flex items-center gap-2">
                <div className="w-12 h-1.5 bg-[#F59E0B]" />
                <div className="w-8 h-1.5 bg-primary" />
                <div className="w-6 h-1.5 bg-[#8B7EC8]" />
              </div>
              <Link to="/register" className="inline-flex items-center gap-1.5 mt-5 text-sm font-bold uppercase tracking-wider text-primary hover:underline">
                Read the Full Text <ArrowUpRight className="h-4 w-4" />
              </Link>
            </motion.div>
            <motion.div {...fadeUp} className="relative">
              <img src={IMG.portrait2} alt="Technology leader" className="w-full aspect-[3/4] object-cover rounded-sm" loading="lazy" />
      ...
      {/* ═══ PROFILE CARD 3 — photo left, same as card 1 pattern ═══ */}
      <section className="py-14 bg-white">
        <div className="max-w-[1400px] mx-auto px-5 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-10 lg:gap-16 items-start">
            <motion.div {...fadeUp} className="relative">
              <img src={IMG.community} alt="Farming community gathering" className="w-full aspect-square object-cover rounded-sm" loading="lazy" />
              <div className="absolute -bottom-2 left-0 w-2/3 h-3 bg-primary/80" />
            </motion.div>
            <motion.div {...fadeUp}>
      ...
      {/* ═══ WIDE PANORAMIC PHOTO with caption ═══ */}
      <section className="relative">
        <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
          <img src={IMG.panorama} alt="West African agricultural landscape panorama" className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
      ...
      {/* ═══ STATS — plain black numbers, thick colored bars, staggered 2-col ═══ */}
      <section className="py-16 md:py-24 bg-[#F5F0EB]">
        <div className="max-w-[1400px] mx-auto px-5 lg:px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-16">
            {/* Row 1 — aligned */}
            <AnimatedCounter target={500} suffix="+" label="Farms managed across West Africa" barColor="primary" />
            <AnimatedCounter target={90} suffix="+" label="Volunteer agricultural experts contributing" barColor="accent-cyan" />
            {/* Row 2 — staggered: first normal, second pushed down */}
            <AnimatedCounter target={12} suffix="" label="Regions and districts covered" barColor="accent-gold" />
            <div className="md:mt-12">
              <AnimatedCounter target={9919915} suffix="" label={'Annual production value tracked on the platform\n\nFeed costs: 56.8% · Medication: 17.9%\nRevenue from eggs: 22.3% · Other: 3%'} barColor="primary" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20 bg-[#F5F0EB]">
        <div className="max-w-[1400px] mx-auto px-5 lg:px-10">
          <h2 className="text-lg md:text-xl font-black uppercase tracking-[0.1em] text-foreground mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
            {sectionCards.map((card) => (
              <Link
                key={card.title}
                to="/register"
                className={`${card.bg} rounded-2xl p-6 md:p-8 aspect-[4/3] flex flex-col justify-between group transition-transform hover:scale-[1.02]`}
              >
                ...
                <div className="self-end">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/20 flex items-center justify-center group-hover:bg-white/30 transition-colors">
        ...
        <div className="max-w-[1400px] mx-auto px-5 lg:px-10 relative z-10 text-center">
          <motion.div {...fadeUp}>
            ...
            <div className="flex items-center justify-center gap-4">
              <Link to="/register" className="inline-flex items-center gap-2 bg-[hsl(var(--accent-gold))] text-foreground font-bold text-sm uppercase tracking-wider px-8 py-3.5 rounded-lg hover:opacity-90 transition-opacity">
