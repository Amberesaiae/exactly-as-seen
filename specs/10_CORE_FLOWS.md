...
| `task_type` | `text` | Enum: `'feed_log' | 'water_log' | 'egg_collection'` |
...
- **Generation:** Generated by `cron_generate_daily_tasks()` at 06:00 farm timezone daily.
- **Conditions:** `egg_collection` tasks are only created for layer Week 19+ and duck-layer Week 20+.
- **Relationship to health_tasks:** Parallel systems, both counted in the main dashboard's `tasks_today_count`.
- **Completion:** Individual per-task tracking (not bulk-completable).
